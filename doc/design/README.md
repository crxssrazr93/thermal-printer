...
| 05 | Soft Glass | Floating frosted panels over a colour wash, pill controls, gradient CTA. | Inter |

## Notes on the choices

Directions 01 to 04 are rated WCAG AAA by the style database. 05 (glassmorphism)
carries a contrast warning, so the panels here use 0.82 opacity rather than the
0.1 that makes glass illegible in light mode.
