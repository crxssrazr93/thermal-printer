# the symbol table
#
# A thermal printer will print any glyph its font carries, which is a great
# many more than a keyboard offers: the mathematics, the Greek, the arrows, the
# set theory. This is that table, kept as plain data so both the printing code
# and the web app can read it, and so adding a symbol needs no code at all.
#
# Each entry is (character, its Unicode name, what it is for). The names and
# descriptions are what makes the picker searchable: nobody remembers U+2A0C,
# but everyone can type "integral".
...
# flattened for search: (symbol, name, description, group)
ALL_SYMBOLS: List[Tuple[str, str, str, str]] = [
    (symbol, name, description, group)
